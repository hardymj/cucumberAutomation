...
const detectchromedriverversion = process.env.npm_config_detectchromedriverversion || process.env.DETECTCHROMEDRIVERVERSION;
let chromedriverversion = process.env.npm_config_chromedriverversion || process.env.CHROMEDRIVERVERSION || helper.version;
let chromedriverBinaryFilePath;
let downloadedFile = '';

(async function install() {
    try {
        if (detectchromedriverversion === 'true') {
...
async function downloadFile(dirToLoadTo) {
    if (detectchromedriverversion !== 'true' && configuredfilePath) {
...
function verifyIfChromedriverIsAvailableAndHasCorrectVersion() {
    ...
    try {
        ...
        const cp = process.spawn(chromedriverBinaryFilePath, ['--version']);
        let str = '';
...
async function getChromeDriverVersion(requestOptions) {
    ...
    chromedriverversion = response.data.trim();
